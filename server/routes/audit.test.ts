...
describe("auditCsv", () => {
  it("writes a header and a row per event", () => {
  ...
  it("uses CRLF between rows, as a CSV reader expects", () => {
    const csv = auditCsv([event(), event({ id: "aud_2" })], sessions);
    expect(csv.split("\r\n")).toHaveLength(3);
  });

  it("survives a command containing commas, quotes and newlines", () => {
