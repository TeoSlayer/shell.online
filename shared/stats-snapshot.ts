...
export function buildFigures(summary: MetricSummaryRow[], audiences: StatsAudiences): StatsFigures {
  return {
    pageLoads: sumCounts(summary, "page_loaded"),
    installsReported: sumCounts(summary, "install_outcome", "ok"),
  ...
  return [
    {
      ...
      unique: people("site"),
      note: "Landing and documentation responses to browser requests, not proof the page rendered. Repeat visits count again.",
      excluded: excluded([
    ...
    {
      key: "loaded",
      label: "Reported a page load",
      count: figures.pageLoads,
      unique: null,
      note: "The public page initialized in the browser. New instrumentation only; blockers, privacy settings and failed JavaScript can prevent this report.",
      excluded: [],
      basis: null,
    },
    {
      key: "signup",
      label: "Clicked a landing-page button",
      count: figures.ctaClicks,
      unique: null,
      note: "Get started, demo and optional account buttons. These are clicks, not sign-ups; see the event breakdown for each button.",
      excluded: [],
      basis: null,
    },
    {
      ...
      unique: null,
      note: "Successful install-command copies, not proof the command was run. Repeat copies count again.",
      excluded: [],
      basis: null,
    },
    {
      key: "installer",
      label: "Fetched the install script",
      count: figures.installerRuns,
      unique: people("install"),
      note: "The script fetched by a command-line client. It may be run, saved, or inspected; fetching alone cannot prove execution.",
      excluded: excluded([
    ...
    {
      key: "installed",
      label: "Downloaded a binary",
      count: figures.installs,
      unique: null,
      note: "Full release binary responses, including direct downloads and repeat requests. Not completed installs; Homebrew/source builds are not covered.",
      excluded: excluded([{ label: "by crawlers", count: audiences.installs.crawlers }]),
      basis: null,
    },
    {
      key: "reported_install",
      label: "Installer reported success",
      count: figures.installsReported,
      unique: null,
      note: "The script reported finishing successfully. Reports can be blocked or disabled; repeats are not unique people. No inferred success from downloads.",
      excluded: [],
      basis: null,
    },
    {
