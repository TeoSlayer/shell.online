...
func TestCreateSessionRequestsAndVerifiesReadOnlyMode(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		if body.Label != "train.py" || body.ReadOnly == nil || !*body.ReadOnly {
	...
	session, err := NewClient(server.URL, "shell/test").CreateSession(context.Background(), "train.py", true)
	if err != nil {
...
func TestCreateSessionRejectsAccessModeMismatch(t *testing.T) {
	...
	_, err := NewClient(server.URL, "shell/test").CreateSession(context.Background(), "train.py", true)
